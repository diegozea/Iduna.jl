...
The generic EPLI workflow starts from an unaligned FASTA or a MIToS MSA object
and a user-provided aligner wrapper. It builds one full reference MSA, samples
sequence rows while keeping the reference sequence, builds one MSA for each
sampled FASTA, scores each sampled MSA against the full MSA, and returns the
median normalized score as `epli`.
...
`epli_score` can also take a `MIToS.MSA.AbstractMultipleSequenceAlignment` as
input. The MSA is used as a source of sequences: EPLI removes `-` and `.` from
each row, writes unaligned FASTA inputs under `workdir`, and then calls the
aligner wrapper as usual.
...
The returned result and `summary.csv` use `input_source = "mitos_msa"` and
`input_fasta = missing` for MSA object input.
